...
mousereport (XEvent *e)
{
    ...
    if (IS_SET (MODE_MOUSESGR))
        {
            len = snprintf (buf, sizeof (buf), "\033[<%d;%d;%d%c", code, x + 1,
    ...
    if (!name)
        {
            if (BETWEEN (i, 16, 255))
                { /* 256 color */
                    ...
                    return XftColorAllocValue (xw.dpy, xw.vis, xw.cmap, &color,
                                               ncolor);
                }
            else
                name = colorname[i];
                    ...
                    for (int code_idx = 0; code_idx < shaped.count; code_idx++)
                        {
                            ...
                            /* Advance the drawing cursor if we've moved to a
                             * new cluster */
                            if (code_idx > 0
                                && idx != shaped.glyphs[code_idx - 1].cluster)
                                {
                                    xp += runewidth;
                                    cluster_xp = xp;
                                    cluster_yp = yp;
                                    ...
                                    specs[numspecs].y
                                        = cluster_yp
                                          - (short)(shaped.positions[code_idx]
                                                    ...
                                                    / 64.);
                                    cluster_xp
                                        += shaped.positions[code_idx].x_advance
                                           / 64.;
                                    cluster_yp
                                        += shaped.positions[code_idx].y_advance
                                           / 64.;
                                    numspecs++;
                                }
                            else
                                    ...
                                    for (f = 0; f < frclen; f++)
                                        {
                                            ...
                                            if (glyphidx
                                                ...
                                                break;
                                            /* We got a default font for a not
                                             * found glyph. */
                                            if (!glyphidx
                                                && frc[f].flags == frcflags
                                                && frc[f].unicodep == rune)
                                    ...
                                    if (f >= frclen)
                                        {
                                             ...
                                             * Now use some dozen of Fontconfig
                                             * calls to get the font for one
                                             * single character.
                                             *
    ...
    if (base.mode & ATTR_UNDERLINE)
        {
            ...
            const int widthThreshold
                = 28; // +1 width every widthThreshold px of font
            ...
            GC ugc = XCreateGC (
                xw.dpy, XftDrawDrawable (xw.draw),
            ...
            if (base.ustyle != 3)
                {
                    ...
                    XFillRectangle (xw.dpy, XftDrawDrawable (xw.draw), ugc,
                                    winx, winy + dc.font.ascent + 1, width,
                                    wlw);
                }
            else if (base.ustyle == 3)
                {
                    ...
                    // Last wave
                    arcs[i * 2] = (XArc){
                        wx + ww * i + ww / 4, wy, ww / 2, wh, 0, 180 * 64
                    };
                    // Last wave tail
                    arcs[i * 2 + 1] = (XArc){ wx + ww * i + ww * 0.75,
                                              wy,
                                              ceil (ww / 2.),
                                              wh,
                                              180 * 64,
                    ...
                    i++;
                    arcs[i * 2]
                        = (XArc){ wx - ww / 4 - 1, wy,     ceil (ww / 2.), wh,
                                  270 * 64,        90 * 64 };
                    ...
                    */
                    ...
                    if (npoints >= 3)
                        {
                            // We add an aditional slot in case we use a bonus
                            // point
                            XPoint *points
                                = xmalloc (sizeof (XPoint) * (npoints + 1));
                            ...
                            // Second point (Goes back to the absolute point
                            // coordinates)
                            points[1] = (XPoint){
                            ...
                            // Draw the lines
                            XDrawLines (xw.dpy, XftDrawDrawable (xw.draw), ugc,
                            ...
                            if (((win.ch / (widthThreshold / 2)) % 2))
                                {
                                    ...
                                    XDrawLines (
                                        xw.dpy, XftDrawDrawable (xw.draw), ugc,
                                        points, npoints, CoordModePrevious);
                                }
...
main (int argc, char *argv[])
{
    ...
    case 'G':
        xw.gm
            = XParseGeometry (EARGF (usage ()), &xw.l, &xw.t, &width, &height);
